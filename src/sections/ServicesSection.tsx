...
const services = [
  ...
  { num: "02", name: "Discord Server Design", desc: "Designing and managing professional Discord servers with well-structured systems, automation, and an enhanced user experience." },
  { num: "03", name: "Custom Discord Bots", desc: "Developing custom bots to automate tasks, improve moderation, and add unique features tailored to your community's needs." },
  { num: "04", name: "Custom Websites", desc: "Building modern, responsive websites tailored to client requirements, complete with secure payment gateways and custom functionality." }
];
          ...
          {services.map((svc, i) => (
            <FadeIn 
              ...
              className={`group p-8 sm:p-10 md:p-12 bg-white/[0.02] backdrop-blur-md shadow-xl border border-white/5 rounded-[30px] sm:rounded-[40px] hover:-translate-y-2 hover:scale-[1.01] hover:bg-white/[0.05] hover:border-white/10 hover:shadow-[0_0_40px_rgba(215,226,234,0.08)] transition-all duration-500 flex flex-col ${
                i === 0 || i === 3 ? 'lg:col-span-2 lg:flex-row lg:items-center lg:justify-between gap-8' : 'gap-8'
              }`}
            >
              <div className={`flex flex-col ${i === 0 || i === 3 ? 'lg:w-[60%]' : ''}`}>
